...
def create_voronoi_regions(garages_df):
    """Create Voronoi regions for garages"""
    points = np.array([[row['longitude'], row['latitude']] for _, row in garages_df.iterrows()])
    vor = Voronoi(points)
    return vor
...
def process_garage(args):
    ...
    G, garage_row, nodes_subset, max_time = args
    try:
        lat = float(garage_row['latitude'])
        lng = float(garage_row['longitude'])
        name = garage_row['name']
...
def create_isochrone_plot(G, garages_df, max_time=60, filename='uk_garage_isochrones_sample.png'):
    # Create Voronoi regions
    vor = create_voronoi_regions(garages_df)

    # Prepare arguments for parallel processing
    process_args = []
    for i, garage in garages_df.iterrows():
...
def plot_garages_voronoi(G, garages_df, filename='uk_garages_voronoi.png'):
    ...
    for _, garage in garages_df.iterrows():
        ax.scatter(float(garage['longitude']), float(garage['latitude']),
                  c='red', s=5, marker='o',
                  label=garage['name'],
                  zorder=3)
...
def main():
    ...
    print(f"Processing {len(garages_df)} garages")

    G = get_uk_network()

    # Plot both visualisations
    plot_garages_voronoi(G, garages_df)
    # create_isochrone_plot(G, garages_df, max_time=60, filename='uk_garage_isochrones_sample.png')
